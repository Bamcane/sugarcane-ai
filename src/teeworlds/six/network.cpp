...
void CNetBase::OpenLog(IOHANDLE DataLogSent, IOHANDLE DataLogRecv)
{
	if(DataLogSent)
	{
		...
		log_msg("network", "logging sent packages");
	}
	else
		log_msg("network", "failed to start logging sent packages");

	if(DataLogRecv)
	{
		ms_DataLogRecv = DataLogRecv;
		log_msg("network", "logging recv packages");
	}
	else
		log_msg("network", "failed to start logging recv packages");
}
...
void CNetBase::CloseLog()
{
	if(ms_DataLogSent)
	{
		log_msg("network", "stopped logging sent packages");
	...
	if(ms_DataLogRecv)
	{
		log_msg("network", "stopped logging recv packages");
